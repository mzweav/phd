...
module Path where
  ...
  idPath : ∀{l}{A : Set l} → (a : A) → Path A a a
  idPath a = (λ _ → a) , id , id
  ...
  apPath : ∀ {l1 l2} {A : Set l1} {B : Set l2} {a0 a1} → (f : A → B) → Path A a0 a1 → Path B (f a0) (f a1)
  apPath f (p , eq0 , eq1) = (λ i → f (p i)) , ap f eq0 , ap f eq1
