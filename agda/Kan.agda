...
module Kan where 
  ...
  hasCom-hasCom2 A hcomA r r' α {{cα}} t α' {{cα'}} t' compat b =
  ...
  hasHCom-relComUnit : {l : Level} (A : Set l) → hasHCom A → relCom (λ (_ : Unit{l}) → A)
  hasHCom-relComUnit A hcomA p i j α {{cα}} t b = fst a , fst (snd a) , λ { id → snd (snd a) id } where

    a : A [ α ↦ t j , i == j ↦ (λ y → fst b) ]
    a = hcomA i j α t b
