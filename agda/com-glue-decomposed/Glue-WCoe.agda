...
module com-glue-decomposed.Glue-WCoe where
  ...
  wcoe-isweq α T B f feq comT comB p r r' = 
    ...
    b'-on-r=r' : (g : G (p r)) (r=r' : r == r') → b' g == unglue {{_}} (transport (G o p) r=r' g)
    b'-on-r=r' g id = ! ((snd (coeB p r r' (unglue {{_}} g))) id)
  
    g-in-fiber-eq : (e : r == r') 
                 (g : Glue _ {{ snd (α (p r)) }} _ _ _)
                 (pα : fst (α (p r'))) 
                 → (g-in-fiber e g pα) =h (Glue-to-fiber {{ cα = snd (α (p r')) }} {f = (f _)} (⇒ g e) pα) 
    g-in-fiber-eq id g pα = transport-=h _ (snd (coeB p r r (unglue {{_}} g)) id)
